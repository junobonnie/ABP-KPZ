...
v_0 = Vector(0, -1)
class Atom(Atom):
    def __init__(self, element, pos, vel = Vector(0, 0), polarity = 0):
...
class Simulator(Simulator):
    ...
    def main(self):
        ...
        count = 0
        for atom in self.world.atoms:
            atom.pos = Vector((x_[count].x+self.render.width/2)%(self.render.width)-self.render.width/2, x_[count].y)
            atom.vel = v_[count]
            count = count + 1
#=============== 새로 추가된 부분 ===============#
...
width = 480 #시뮬레이션 공간의 가로길이
height = 360 #시뮬레이션 공간의 세로길이
...
while True:
    ...
    # simulator.save_screen(저장위치, 건너뛸 개수(디폴트 0))
    # simulator.save_snapshot('snapshots/ABP/')
    ### 매시간 마다 시뮬레이션 스냅샷을 저장하는 코드 ###